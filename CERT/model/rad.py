import pandas as pd
import torch
from CERT.model import a2c, protonet
...
#                 prototrainer.training_baseline(df_seen, df_seen_eval, df_unseen, test_x, test_y)
            ...
            if len(df_unseen_pseudo.groupby('y_pred').count()['y_true']) == 1:
                model_final = protonet.ProtoTrainer(self.options)
                model_final.final_training_testing(df_seen, test_x, test_y)
            ...
            for _ in tqdm(range(self.max_episode), desc="iterator {:d} train".format(i_iterator + 1)):
                ...
                loss = self.finish_episode(abs(r_episode - r_before))
